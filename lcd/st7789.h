...
static const uint16_t st7789_init_seq[] = {
    ...
    2, 0, 0xc0, 0x2c,                          // Set MADCTL: row then column, refresh is bottom to top ????
    2, 0, 0xc2, 0x01,                          // CASET: column addresses from 0 to 240 (f0)
    2, 0, 0xc3, 0x12,                          // RASET: row addresses from 0 to 240 (f0)
    2, 2, 0xc4, 0x20,                          // Inversion on, then 10 ms delay (supposedly a hack?)
    2, 2, 0xc6, 0x0f,
    3, 10, 0xd0, 0xa4, 0xa1,
    2, 1, 0xd6, 0xa1,
    15, 20, 0xe0, 0xd0, 0x08, 0x11, 0x08, 0x0c, 0x15, 0x39, 0x33, 0x50, 0x36, 0x13, 0x14, 0x29, 0x2d,
    15, 20, 0xe0, 0xd0, 0x08, 0x10, 0x08, 0x06, 0x6, 0x39, 0x44, 0x51, 0x0b, 0x16, 0x14, 0x2f, 0x31,
...
static inline void lcd_set_dc_cs(lcd *screen, bool dc, bool cs)
{
    sleep_us(1);
    gpio_put_masked((1u << screen->PIN_DC) | (1u << screen->PIN_CS), !!dc << screen->PIN_DC | !!cs << screen->PIN_CS);
    sleep_us(1);
}

static inline void lcd_write_cmd(lcd *screen, const uint16_t *cmd, size_t count)
{
    st7789_lcd_wait_idle(screen->pio, screen->sm);
    lcd_set_dc_cs(screen, 0, 0);
    st7789_lcd_put(screen->pio, screen->sm, *cmd++);
    if (count >= 2)
    {
        st7789_lcd_wait_idle(screen->pio, screen->sm);
        lcd_set_dc_cs(screen, 1, 0);
        for (size_t i = 0; i < count - 1; ++i)
            st7789_lcd_put(screen->pio, screen->sm, *cmd++);
    }
    st7789_lcd_wait_idle(screen->pio, screen->sm);
    lcd_set_dc_cs(screen, 1, 1);
}

static inline void lcd_init(lcd *screen, const uint16_t *init_seq)
{
    const uint16_t *cmd = init_seq;
    while (*cmd)
    {
        lcd_write_cmd(screen, cmd + 2, *cmd);
        sleep_ms(*(cmd + 1) * 5);
        cmd += *cmd + 2;
    }
}
...
static inline void st7789_start_pixels(lcd *screen)
{
    ...
    lcd_write_cmd(screen, &cmd, 1);
    lcd_set_dc_cs(screen, 1, 0);
}

void backlight(lcd *screen, int en)
{
    gpio_put(screen->PIN_BL, en);
}

void attach(lcd *screen)
{
    stdio_init_all();
    uint offset = pio_add_program(screen->pio, &st7789_lcd_program);
    st7789_lcd_program_init(screen->pio, screen->sm, offset, screen->PIN_DIN, screen->PIN_CLK, SERIAL_CLK_DIV);
    ...
    lcd_init(screen, st7789_init_seq);
    backlight(screen, 1);
}

void update(lcd *screen, uint16_t p[])
{
    ...
    case HORIZONTAL:
        for (int x = 0; x < SCREEN_HEIGHT; ++x)
        {
            for (int y = SCREEN_WIDTH - 1; y >= 0; --y)
            {
                ...
                st7789_lcd_put(screen->pio, screen->sm, p[x + y * SCREEN_HEIGHT] & 0xff);
            }
        }
        break;
